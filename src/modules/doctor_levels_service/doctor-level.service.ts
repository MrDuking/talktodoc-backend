...
export class DoctorLevelService {
  ...
  async createDoctorLevel(createDoctorLevelDto: CreateDoctorLevelDto): Promise<DoctorLevel> {
    try {
      const existingLevel = await this.doctorLevelModel
        .findOne({ name: createDoctorLevelDto.name })
        .exec()
      if (existingLevel) {
  ...
  async updateDoctorLevel(
    id: string,
    updateDoctorLevelDto: UpdateDoctorLevelDto,
  ): Promise<DoctorLevel> {
    if (!Types.ObjectId.isValid(id)) {
      throw new BadRequestException('Invalid doctor level ID format')
    }

    const updatedLevel = await this.doctorLevelModel
      .findByIdAndUpdate(id, updateDoctorLevelDto, { new: true })
      .exec()
